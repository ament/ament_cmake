      ...
      elseif(TARGET "${_lib}")
        ...
        if(_is_imported)
          ...
          if(_imported_location)
            ...
            endforeach()
            if(NOT _imported_libraries)
              get_target_property(_location "${_lib}" LOCATION)
              if(_location)
                list(APPEND _imported_libraries ${_location})
              else()
                foreach(_cfg ${_imported_configurations})
                  get_target_property(_location_${_cfg} "${_lib}"
                    LOCATION_${_cfg})
                  if(_location_${cfg})
                    list(APPEND _imported_libraries ${_location_${_cfg}})
                  endif()
                endforeach()
              endif()
            endif()
