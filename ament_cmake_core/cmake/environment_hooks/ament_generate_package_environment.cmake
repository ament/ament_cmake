...
function(ament_generate_package_environment)
  ...
  if(DEFINED _AMENT_CMAKE_ENVIRONMENT_PREPEND_NON_DUPLICATE)
    foreach(entry ${_AMENT_CMAKE_ENVIRONMENT_PREPEND_NON_DUPLICATE})
      string(REPLACE "|" ";" parts "${entry}")
      list(GET parts 0 condition)
      list(GET parts 1 var)
      list(GET parts 2 value)

      if("${condition}" STREQUAL "ALWAYS")
        set(all_hooks "${all_hooks}prepend-non-duplicate;${var};${value}\n")
      elseif("${condition}" STREQUAL "IF_EXISTS")
        set(all_hooks "${all_hooks}prepend-non-duplicate-if-exists;${var};${value}\n")
      else()
        message(FATAL_ERROR "Unknown condition '${condition}' in _AMENT_CMAKE_ENVIRONMENT_PREPEND_NON_DUPLICATE")
      endif()
    endforeach()
  endif()
